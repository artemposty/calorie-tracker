...
export function useWeight() {
  ...
  const setWeight = useCallback((date: string, weight: number) => {
    setEntries(prev => {
      ...
      if (exists) {
        sb(supabase.from('weight_entries').update({ weight }).eq('id', exists.id));
        return prev.map(e => e.date === date ? { ...e, weight } : e);
      }
      const id = newId();
      sb(supabase.from('weight_entries').insert({ id, user_id: USER_ID, date, weight }));
  ...
  const deleteEntry = useCallback((id: string) => {
    ...
    sb(supabase.from('weight_entries').delete().eq('id', id));
  }, []);
  ...
  const getStats = useCallback(() => {
    ...
    return { min: Math.min(...allWeights), max: Math.max(...allWeights), start, current, weekTrend, count: sorted.length };
  }, [entries]);

  const replaceAll = useCallback(async (newEntries: WeightEntry[]) => {
    const { error: delErr } = await supabase.from('weight_entries').delete().eq('user_id', USER_ID);
    if (delErr) console.error('[supabase replaceAll delete]', delErr);
    if (newEntries.length > 0) {
      const { error: insErr } = await supabase.from('weight_entries').insert(
