...
function CustomDot(props: { cx?: number; cy?: number; payload?: DayStats; goal: number }) {
  ...
  const over = payload.kcal > goal;
  return <circle cx={cx} cy={cy} r={4} fill={over ? '#ef4444' : '#22c55e'} stroke="white" strokeWidth={1.5} />;
}
...
export function CalorieChart({ data, goals }: Props) {
  ...
  const maxKcal = Math.max(...data.map(d => d.kcal), goals.kcal);
    ...
    <div className="bg-white rounded-2xl p-4 shadow-sm">
      ...
      <div className="flex items-center gap-3 mb-3">
        <span className="flex items-center gap-1 text-xs text-slate-400">
          <span className="w-2.5 h-2.5 rounded-full bg-green-400 inline-block" /> в норме
        </span>
        <span className="flex items-center gap-1 text-xs text-slate-400">
      ...
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={data} margin={{ top: 4, right: 8, bottom: 0, left: -8 }}>
          <ReferenceArea y1={0} y2={goals.kcal} fill="#dcfce7" fillOpacity={0.6} ifOverflow="hidden" />
          <ReferenceArea y1={goals.kcal} y2={yMax} fill="#fee2e2" fillOpacity={0.5} ifOverflow="hidden" />
