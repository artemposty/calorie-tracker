...
export function CalorieChart({ data, goals, baseTdee, tonnageByDate }: Props) {
  ...
  const chartData = data.map(d => {
    const exp = calcExpenditure(baseTdee, calcWorkoutKcal(tonnageByDate[d.date] ?? 0));
    return { displayDate: d.displayDate, kcal: d.kcal, exp, date: d.date };
  });
    ...
    <div className="rounded-2xl p-4" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <ComposedChart data={chartData} margin={{ top: 4, right: 8, bottom: 0, left: -8 }}>
          {/* ── Dynamic expenditure fill (amber from 0→exp per day) ── */}
          <Area dataKey="exp" fill="rgba(255,159,10,0.05)" stroke="none"
            type="monotone" isAnimationActive={false} />

          {/* ── Static undereating zones overlay (cover amber below goal) */}
          <ReferenceArea y1={0}         y2={redLow}    fill="rgba(255,69,58,0.07)"  ifOverflow="hidden" />
          <ReferenceArea y1={redLow}    y2={yellowLow} fill="rgba(255,159,10,0.06)" ifOverflow="hidden" />
          <ReferenceArea y1={yellowLow} y2={goal}      fill="rgba(48,209,88,0.07)"  ifOverflow="hidden" />
          ...
          {/* ── Expenditure boundary as thin dashed line ──────── */}
          <Line type="monotone" dataKey="exp" stroke="rgba(255,158,10,0.30)"
            strokeDasharray="4 3" strokeWidth={1} dot={false} activeDot={false}
            isAnimationActive={false} />

          {/* ── Intake line with zone-colored dots ────────────── */}
          <Line type="monotone" dataKey="kcal" stroke="rgba(255,255,255,0.55)" strokeWidth={2}
            dot={(props: Record<string, unknown>) => {
              ...
              return <circle key={`d-${cx}`} cx={cx} cy={cy} r={4}
                fill={dotColor(payload.kcal, payload.exp)} stroke="var(--bg-card)" strokeWidth={2} />;
            }}
